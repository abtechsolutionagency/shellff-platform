...
const GroupPacksList: React.FC<GroupPacksListProps> = ({ releaseId }) => {
  ...
  const loadGroupPacks = useCallback(async () => {
    try {
      setLoading(true);
  ...
  }, [currentPage, filterPackType, filterStatus, releaseId]);

  useEffect(() => {
    void loadGroupPacks();
  }, [loadGroupPacks]);

  const handleTogglePackStatus = async (_packId: string, currentStatus: boolean) => {
    // TODO: Implement pack status toggle API
    toast.success(`Pack ${currentStatus ? 'deactivated' : 'activated'} successfully`);
  };

  const handleDeletePack = async (_packId: string) => {
  ...
  const getPackTypeIcon = (packType: string): ReactNode => {
    const iconMap: Record<string, ReactNode> = {
      family: <Users className="h-4 w-4 text-purple-500" aria-hidden="true" />,
      friends: <Users className="h-4 w-4 text-sky-500" aria-hidden="true" />,
      bulk: <Package className="h-4 w-4 text-orange-500" aria-hidden="true" />,
      corporate: <Package className="h-4 w-4 text-teal-500" aria-hidden="true" />
    };

    return iconMap[packType as keyof typeof iconMap] ?? (
                ...
                {pack.discountPercentage > 0 && (
                  <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                    <p className="text-sm text-green-700 dark:text-green-300">
                      Members save {pack.discountPercentage}% with this group pack! (Original: {pack.originalPrice} SHC, Now: {pack.discountedPrice} SHC)
                    </p>
                  </div>
                )}
              </CardContent>
