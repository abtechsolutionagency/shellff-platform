...
import { ArrowLeft, Edit, Music2, Users, Coins, MoreHorizontal, Play, Plus, ShoppingBag, Scan, FileText, TrendingUp } from "lucide-react";
...
import { SHCBalanceCard } from "../dashboard/SHCBalanceCard";
...
const mockPurchases = [
  {
    ...
    purchaseMethod: 'barcode' as const,
    price: 2.5,
...
export function LibraryContent() {
  ...
  const handleAddToShellff = (albumPreview: any) => {
    console.log('Added to My Shellff:', albumPreview);
    // You can add a success toast or update state here
  };

  const userType = (session?.user as any)?.userType;
  const isCreator = userType === 'CREATOR' || userType === 'Creator' || userType === 'creator';

  const getPurchaseMethodIcon = (method: string) => {
    switch (method) {
      case 'barcode':
  ...
  const getPurchaseMethodLabel = (method: string) => {
    switch (method) {
      case 'barcode':
        return 'Barcode Scan';
      case 'code':
          ...
          {mockPurchases.length === 0 ? (
            <Card className="bg-gray-900/50 border-gray-800">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-400 mb-4">
                  Start exploring music by scanning barcodes or entering codes to add albums to your collection.
                </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {mockPurchases.map((purchase) => (
                <Card key={purchase.id} className="bg-gray-900/50 border-gray-800 hover:bg-gray-800/50 transition-all duration-200 group cursor-pointer">
                    ...
                    <div className="relative aspect-square mb-4 rounded-lg overflow-hidden bg-gray-800">
                      ...
                      <div className="absolute top-2 right-2">
                        <Badge 
                          ...
                          className={purchase.status === 'active' ? 'bg-green-600 hover:bg-green-600' : ''}
                        >
                          {purchase.status}
                        </Badge>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        <div className="flex items-center gap-1">
                          {getPurchaseMethodIcon(purchase.purchaseMethod)}
                          <span>{getPurchaseMethodLabel(purchase.purchaseMethod)}</span>
                        </div>
                        <span>{purchase.price} SHC</span>
                      </div>
            ...
            {isCreator ? (
              ...
              <>
                <div className="flex items-center gap-4 p-4 bg-gray-900/50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-white text-sm">
                      <span className="text-purple-400">Published</span> new track "Digital Pulse"
                    </p>
                ...
                <div className="flex items-center gap-4 p-4 bg-gray-900/50 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-white text-sm">
                      <span className="text-yellow-400">Achieved</span> 1,000 streams milestone for "Eclipse Dreams"
                    </p>
