...
import { Home, Library, Wallet, User, Music, Gift, Users, Shield, BarChart3, Package, DollarSign } from "lucide-react";
...
export function MobileBottomNav() {
  ...
  const navItems = [
    ...
    ...(isAdmin ? [
      ...
      // Library is available for users with listener capabilities
      {
        ...
        show: canAccessListenerFeatures
      },
      {
        icon: Gift,
        label: "Unlock",
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black/95 backdrop-blur-sm border-t border-gray-800">
      <div className={`grid h-16 max-w-full overflow-hidden ${navItems.length === 5 ? 'grid-cols-5' : navItems.length === 4 ? 'grid-cols-4' : 'grid-cols-3'}`}>
        {navItems.map((item) => {
